...
void ApexGPUModel::reset() {
    for (auto& reg : registers_) reg.fill(0);
    counters_ = {};
}

void ApexGPUModel::set_register(uint8_t index, const Vector& value) {
    if (index >= kRegisters) throw std::out_of_range("register index");
    registers_[index] = value;
}

const Vector& ApexGPUModel::get_register(uint8_t index) const {
    if (index >= kRegisters) throw std::out_of_range("register index");
...
int32_t ApexGPUModel::apply(Opcode opcode, int32_t a, int32_t b, int32_t immediate) {
    switch (opcode) {
        ...
        case Opcode::VABSDELTA: {
            ...
            uint64_t magnitude = static_cast<uint64_t>(d < 0 ? -d : d);
            return wrap_u32(static_cast<uint32_t>(magnitude));
        }
        case Opcode::VCLAMP: {
...
void ApexGPUModel::execute(const Instruction& instruction) {
    ...
    const Vector a = registers_[instruction.src_a];
    const Vector b = registers_[instruction.src_b];
    Vector result = registers_[instruction.dst];
    ...
    if (instruction.opcode != Opcode::VNOP) {
        ...
        registers_[instruction.dst] = result;
    }
    ++counters_.instructions;
}

std::string ApexGPUModel::dump_register(uint8_t index) const {
    const auto& r = get_register(index);
    std::ostringstream out;
    out << "v" << static_cast<int>(index) << " = [";
